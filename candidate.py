...
def predict_career(responses, domain_data):
    ...
    for domain_info in domain_data:
        domain_responses = np.array([domain_info.get(f"q{i}", 0) for i in range(1, 16)])
        distance = euclidean(responses, domain_responses)
        
        # Count matching responses
        matches = np.sum(responses == domain_responses)
        ...
        print(f"Distance: {distance}, Matches: {matches}")
        
        if matches >= 13:
            matched_domains["exact"].append((domain_info['domain'], matches))
        elif matches >= 9:
            matched_domains["close"].append((domain_info['domain'], matches))
    
    # Sort by number of matches
    for key in matched_domains:
        matched_domains[key].sort(key=lambda x: x[1], reverse=True)
...
if __name__ == "__main__":
    ...
    if responses:
        ...
        matched_domains = predict_career(responses, domain_data)
        
        print("\nSuggested Career Domains:")
        best_match = None
        if matched_domains["exact"]:
            best_match = matched_domains["exact"][0][0]
            print("Exact Matches (13-15 out of 15):")
            for domain, matches in matched_domains["exact"]:
                print(f"Domain: {domain}, Matches: {matches}")
        if matched_domains["close"]:
            if not best_match:
                best_match = matched_domains["close"][0][0]
            print("\nClose Matches (9 or more out of 15):")
            for domain, matches in matched_domains["close"]:
                print(f"Domain: {domain}, Matches: {matches}")
        
        # Store user responses and best match domain in MongoDB
        if best_match:
